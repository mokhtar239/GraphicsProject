	...
	case WM_LBUTTONUP:
		if (isDrawing) {
			...
			if (currentMode == MODE_SET_CLIP_RECT) {
				...
				globalClipRegion.xmin = min(startX, endX);
				globalClipRegion.ymin = min(startY, endY);
				globalClipRegion.xmax = max(startX, endX);
				globalClipRegion.ymax = max(startY, endY);
			}
			else if (currentMode == MODE_SET_CLIP_SQUARE) {
				globalClipRegion.shape = CLIP_SQUARE;
				int side = max(std::abs(endX - startX), std::abs(endY - startY));
				int signX = (endX > startX) ? 1 : -1;
				int signY = (endY > startY) ? 1 : -1;
				globalClipRegion.xmin = min(startX, startX + signX * side);
				globalClipRegion.ymin = min(startY, startY + signY * side);
				globalClipRegion.xmax = max(startX, startX + signX * side);
				globalClipRegion.ymax = max(startY, startY + signY * side);
			}
				...
				if (currentShapeType == SHAPE_BONUS_HAPPY || currentShapeType == SHAPE_BONUS_SAD) {
					int cx = (startX + endX) / 2;
					int cy = (startY + endY) / 2;
					int r = (int)std::round(std::sqrt(std::pow(endX - startX, 2) + std::pow(endY - startY, 2))) / 2;
					if (r > 10) {
						if (currentShapeType == SHAPE_BONUS_HAPPY)
							DrawHappyFace(memDC, cx, cy, r, currentColor, bgColor);
						else
							DrawSadFace(memDC, cx, cy, r, currentColor, bgColor);

						Shape fs;
						fs.type = currentShapeType;
						fs.x1 = cx; fs.y1 = cy;
						fs.x2 = r;  fs.y2 = r;
						fs.color = currentColor;
						fs.clipRegion = globalClipRegion;
						shapes.push_back(fs);
					}
				}
				else {
	...
	case WM_PAINT: {
		...
		if (isDrawing) {
			if (currentMode == MODE_SET_CLIP_RECT) {
				...
				Rectangle(hdc, min(startX, currentX), min(startY, currentY), max(startX, currentX), max(startY, currentY));
				SelectObject(hdc, o_rb); DeleteObject(rb);
			}
			else if (currentMode == MODE_SET_CLIP_SQUARE) {
				HPEN rb = CreatePen(PS_DOT, 1, RGB(0, 0, 0)); HPEN o_rb = (HPEN)SelectObject(hdc, rb);
				SelectObject(hdc, GetStockObject(NULL_BRUSH));
				int side = max(std::abs(currentX - startX), std::abs(currentY - startY));
				...
				if (currentShapeType == SHAPE_FILL_BEZIER) {
					...
					Rectangle(hdc, min(startX, currentX), min(startY, currentY),
						max(startX, currentX), max(startY, currentY));
					...
					int side = max(A, B);
					int xm = min(startX, currentX);
					int ym = min(startY, currentY);
				...
				else if (currentShapeType == SHAPE_BONUS_HAPPY || currentShapeType == SHAPE_BONUS_SAD) {
					HPEN pen = CreatePen(PS_DOT, 1, currentColor);
					HPEN oldPen = (HPEN)SelectObject(hdc, pen);
					SelectObject(hdc, GetStockObject(NULL_BRUSH));
					int r = (int)std::round(std::sqrt(std::pow(currentX - startX, 2) + std::pow(currentY - startY, 2))) / 2;
					int cx = (startX + currentX) / 2;
					int cy = (startY + currentY) / 2;
